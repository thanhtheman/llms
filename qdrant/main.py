...
# as the chunks are documents, we use from_documents method, we only run this ONCE to add the youtube transcript
# create_vector_store = Qdrant.from_documents(get_chunks(url), embeddings, url=os.getenv("QDRANT_URL"), api_key=os.getenv("QDRANT_API_KEY"), collection_name="first-collection")
